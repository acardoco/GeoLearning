...
def getImage(filename):
    ...
    with tf.name_scope('decode_jpeg',[image_buffer], None):
        # decode
        # CAMBIO añadido resize_images y set_shape
        image = tf.image.resize_images(tf.image.decode_jpeg(image_buffer, channels=3), [height, width])
...
#CAMBIO ruta cambiada a mano
label, image = getImage("data\\train-00000-of-00002.tfrecord")

record_iterator = tf.python_io.tf_record_iterator(path="data\\train-00000-of-00002.tfrecord")
# and similarly for the validation data
#CAMBIO ruta cambiada a mano
vlabel, vimage = getImage("data\\validation-00000-of-00001.tfrecord")
...
imageBatch, labelBatch = tf.train.shuffle_batch(
    ...
    capacity=2000,
    min_after_dequeue=1000,num_threads=3)

# and similarly for the validation data 
vimageBatch, vlabelBatch = tf.train.shuffle_batch(
    [vimage, vlabel], batch_size=500,
    capacity=2000,
    min_after_dequeue=1000,num_threads=3)
